...
(function installPhaseD4(){
  ...
  function structureTelemetry(pop){
    const count=x=>E().structureHealth(x.program).filled||x.program.slice(1).filter(Boolean).length;
    const cs=pop.map(count),sorted=cs.slice().sort((a,b)=>a-b),ranked=pop.slice().sort((a,b)=>stronger(a,b)?-1:stronger(b,a)?1:0),top=ranked.slice(0,Math.min(30,ranked.length)).map(count);
    const ops={};let multiIndividuals=0,multiSteps=0;
    for(const x of pop){const parts=String(x.operator||'').split('+').filter(Boolean);if(parts.length>1){multiIndividuals++;multiSteps+=parts.length;}for(const op of parts)ops[op]=(ops[op]||0)+1;}
    const buckets={'6':0,'7-9':0,'10-12':0,'13-15':0,'16-18':0,'19+':0};
    for(const n of cs){if(n<=6)buckets['6']++;else if(n<=9)buckets['7-9']++;else if(n<=12)buckets['10-12']++;else if(n<=15)buckets['13-15']++;else if(n<=18)buckets['16-18']++;else buckets['19+']++;}
    const avg=a=>a.length?a.reduce((s,v)=>s+v,0)/a.length:0;
    return{avg:avg(cs),min:sorted[0]||0,max:sorted[sorted.length-1]||0,top30Avg:avg(top),buckets,operators:ops,multiMutationIndividuals:multiIndividuals,multiMutationAvgSteps:multiIndividuals?multiSteps/multiIndividuals:0};
  }
  ...
  async function run(){
    ...
    try{
      for(let g=0;g<GENERATIONS;g++){
        ...
        const ea=aggregate(pop,'engagement'),sa=aggregate(pop,'eval'),feasible=pop.filter(x=>engageFeasible(x.engagement)).length,feasibleByCluster=gs.map(z=>z.filter(x=>engageFeasible(x.engagement)).length),structure=structureTelemetry(pop);
        let checkpoint=null;
        ...
        log.push({generation:g+1,feasible,feasibleRate:feasible/POP,feasibleByCluster,engagement:{attackGameRate:ea.attackGameRate,damageGameRate:ea.damageGameRate,nonCombatRate:ea.nonCombatRate,avgAttacks:ea.avgAttacks,avgDamage:ea.avgDamage},strength:{winRate:sa.winRate,resolvedRate:sa.resolvedRate,avgDamage:sa.avgDamage},structure,checkpoint:checkpoint?{generation:checkpoint.generation,generalizationRate:checkpoint.generalizationRate,falseFeasibleRate:checkpoint.falseFeasibleRate,championValidation:{winRate:checkpoint.champion.metrics.winRate,avgDamage:checkpoint.champion.metrics.avgDamage,resolvedRate:checkpoint.champion.metrics.resolvedRate}}:null,clusters:gs.map(z=>z.length),battles:c.battles});
        phaseProgress.style.width=(((g+1)/GENERATIONS)*95).toFixed(1)+'%';phaseSummary.textContent=`世代 ${g+1}/${GENERATIONS} / 対戦 ${c.battles} / Engagement可 ${feasible}/${POP} (${pct(feasible/POP)}) / チップ 平均${structure.avg.toFixed(1)}・${structure.min}-${structure.max} / 上位30平均${structure.top30Avg.toFixed(1)}${checkpoint?` / Checkpoint Val勝率 ${pct(checkpoint.champion.metrics.winRate)} / best=${bestCheckpoint.generation}世代`:''}`;
      ...
      window.__phaseD4Report=report;phaseReport.textContent=JSON.stringify(report,null,2);phaseStatus.textContent=report.pass?'Phase D4 PASS：Validation checkpoint選択と完全未使用Testを完走しました。':'Phase D4 要確認：監査またはcheckpoint/Test条件に問題があります。';phaseProgress.style.width='100%';const lastStructure=log[log.length-1]?.structure;phaseSummary.textContent=`選択Checkpoint ${bestCheckpoint.generation}世代 / Validation勝率 ${pct(bestCheckpoint.champion.metrics.winRate)} / Test勝率 ${pct(testMetrics.winRate)} / Test平均Damage ${testMetrics.avgDamage.toFixed(2)} / 構造最終 平均${lastStructure?.avg.toFixed(1)}・最大${lastStructure?.max}・上位30平均${lastStructure?.top30Avg.toFixed(1)} / ${report.pass?'PASS':'要確認'}`;return report;
