...
(function installStructuralEvolutionPrototype(){
  const VERSION='grid-native-structure-v0.4-metadata';
  const cfg={
    minReachable:6,
    ...
    disabledOperators:['redirectEdge','extendBranch'],
    operatorWeights:{
      replaceAction:.20,
      replaceCondition:.16,
      insertActionRepack:.16,
      insertConditionBranch:.22,
      extendBranch:0,
      removeActionRepack:.10,
      collapseCondition:.06,
      weaponMutation:.10
    }
  };

  const catalog=()=>window.__chipCatalog||null;
  function allTypes(kind=null){if(catalog())return kind?catalog().chipTypesByKind(kind):[...catalog().registry.values()].map(d=>d.type);return chipTypes.filter(x=>!kind||x[2]===kind).map(x=>x[0]);}
  function meta(type){if(catalog())return catalog().chipDefinition(type);const t=chipTypes.find(x=>x[0]===type);return t?{type:t[0],kind:t[2],category:t[2]==='cond'?'sensor':'state',subCategory:'other',capabilities:t[2]==='cond'?['sensor']:[]}:null;}
  function compatible(type){if(catalog())return catalog().compatibleChipTypes(type);const m=meta(type);return m?allTypes(m.kind).filter(x=>x!==type):[];}
  function provides(type,cap){if(catalog())return catalog().chipProvides(type,cap);const m=meta(type);return cap==='sensor'?m?.kind==='cond':m?.category===cap;}
  function insertActionTypes(){return allTypes('action').filter(t=>['movement','orientation','weapon'].includes(meta(t)?.category));}
  function conditionTypes(){return allTypes('cond');}
  function weaponActionTypes(){return allTypes('action').filter(t=>meta(t)?.category==='weapon');}
  ...
  function structureHealth(p){const r=reachable(p),ids=[...r].filter(i=>i>0&&p[i]),types=ids.map(i=>p[i].type),broken=[];for(const i of ids){const c=p[i];for(const f of c.kind==='action'?['next']:['yes','no']){const j=nextCell(i,c[f]);if(j===i||(j!==0&&!p[j]))broken.push({index:i,field:f});}}const functions={};for(const k of cfg.requiredFunctions)functions[k]=types.some(t=>provides(t,k));const limit=typeof cpuChipLimit==='function'?cpuChipLimit():18;const valid=ids.length>=cfg.minReachable&&cfg.requiredFunctions.every(k=>functions[k])&&broken.length===0&&chipCount(p)<=limit;return{valid,reachable:ids.length,filled:chipCount(p),limit,functions,broken:broken.length};}
  function actionGroup(type){const m=meta(type);if(!m||m.kind!=='action')return null;return[type,...compatible(type)];}
  function conditionGroup(type){const m=meta(type);if(!m||m.kind!=='cond')return null;return[type,...compatible(type)];}
  ...
  function randomGrowAction(rng=Math.random){return pick(insertActionTypes(),rng);}

  function replaceAction(p,rng=Math.random){const n=clone(p),ids=[...reachable(n)].filter(i=>i>0&&n[i]?.kind==='action'),i=pick(ids,rng);if(i==null)return null;const group=actionGroup(n[i].type);if(!group||group.length<2)return null;const type=pick(group.filter(x=>x!==n[i].type),rng);if(!type)return null;n[i]={...n[i],type};return{program:n,detail:{index:i,type,category:meta(type)?.category,subCategory:meta(type)?.subCategory}};}
  function replaceCondition(p,rng=Math.random){const n=clone(p),ids=[...reachable(n)].filter(i=>i>0&&n[i]?.kind==='cond'),i=pick(ids,rng);if(i==null)return null;const group=conditionGroup(n[i].type);if(!group||group.length<2)return null;const type=pick(group.filter(x=>x!==n[i].type),rng);if(!type)return null;n[i]={...n[i],type};return{program:n,detail:{index:i,type,category:meta(type)?.category,subCategory:meta(type)?.subCategory}};}
  ...
  function solveLocalLayout(p,movable,removed,extraNodes,oldEdges,extraEdges,rng=Math.random){const removedSet=new Set(removed||[]),movableSet=new Set(movable||[]),extraNames=Object.keys(extraNodes||{}),entities=[...movableSet,...extraNames];const fixedOccupied=new Set();for(let i=1;i<36;i++)if(p[i]&&!movableSet.has(i)&&!removedSet.has(i))fixedOccupied.add(i);const incident=new Map();for(const e of [...oldEdges,...extraEdges]){for(const x of [e.from,e.to]){if(x===0)continue;if(!incident.has(x))incident.set(x,[]);incident.get(x).push(e);}}const origins=[...movableSet],center=origins.length?origins:extraNames.map(()=>1),allowedCells=[];for(let pos=1;pos<36;pos++){if(fixedOccupied.has(pos))continue;if(center.some(o=>manhattan(pos,o)<=cfg.localRepackRadius)||extraNames.length&&extraEdges.some(e=>(e.from===extraNames[0]||e.to===extraNames[0])&&typeof(e.from===extraNames[0]?e.to:e.from)==='number'&&manhattan(pos,e.from===extraNames[0]?e.to:e.from)<=2))allowedCells.push(pos);}const candidates=new Map();for(const ent of entities){let cands=allowedCells.slice(),fixedAdj=[];for(const e of incident.get(ent)||[]){const other=e.from===ent?e.to:e.from;if(typeof other==='number'&&other>=0&&!movableSet.has(other)&&!removedSet.has(other)){if(other===0)cands=cands.filter(x=>x===1);else fixedAdj.push(other);}}if(fixedAdj.length)cands=cands.filter(x=>fixedAdj.every(y=>adjacent(x,y)));if(typeof ent==='number')cands.sort((a,b)=>manhattan(a,ent)-manhattan(b,ent));else cands=shuffled(cands,rng);candidates.set(ent,cands);}const degree=ent=>(incident.get(ent)||[]).length,order=entities.slice().sort((a,b)=>candidates.get(a).length-candidates.get(b).length||degree(b)-degree(a)),assign=new Map(),used=new Set();let layouts=0;function posOf(x){if(assign.has(x))return assign.get(x);if(typeof x==='number'&&!movableSet.has(x)&&!removedSet.has(x))return x;return null;}function consistent(ent,pos){if(used.has(pos))return false;for(const e of incident.get(ent)||[]){const other=e.from===ent?e.to:e.from;if(other===0){if(pos!==1)return false;continue;}const op=posOf(other);if(op!=null&&!adjacent(pos,op))return false;}return true;}function dfs(k){if(layouts++>cfg.localRepackLayouts)return false;if(k>=order.length)return true;const ent=order[k];for(const pos of candidates.get(ent)){if(!consistent(ent,pos))continue;assign.set(ent,pos);used.add(pos);if(dfs(k+1))return true;used.delete(pos);assign.delete(ent);}return false;}return dfs(0)?assign:null;}
  ...
  function insertConditionBranch(p,rng=Math.random,forcedCondition=null,forcedAction=null){if(chipCount(p)>=(typeof cpuChipLimit==='function'?cpuChipLimit():18))return null;const n=clone(p),acts=[...reachable(n)].filter(i=>i>0&&n[i]?.kind==='action'&&emptyNeighbors(n,i).length),i=pick(acts,rng);if(i==null)return null;const old={...n[i]},oldDest=nextCell(i,old.next);if(oldDest===i||oldDest<0||oldDest>=36||(!n[oldDest]&&oldDest!==0))return null;const empty=pick(emptyNeighbors(n,i),rng),condType=forcedCondition||pick(conditionTypes(),rng),actionType=forcedAction||old.type;if(!condType)return null;n[i]={type:condType,kind:'cond',yes:empty.dir,no:old.next};n[empty.index]={type:actionType,kind:'action',next:dirBetween(empty.index,i)||'L'};return{program:n,detail:{replacedAction:i,condition:condType,branchAction:empty.index,actionType,oldDest}};}

  function removeNodeRepack(p,target,successor,rng=Math.random){if(target<=1||!p[target]||successor<=0||!p[successor])return null;const preds=incoming(p,target).filter(x=>x.index>0);if(!preds.length)return null;const fixed=[...new Set(preds.map(x=>x.index)),target],sets=movableSets(p,successor,fixed,rng);for(const s of sets){const predKeys=new Set(preds.map(x=>x.index+'|'+x.field)),preserved=edgeRecords(p).filter(e=>e.from!==target&&e.to!==target&&!predKeys.has(e.from+'|'+e.field)),extraEdges=preds.map(x=>({from:x.index,field:x.field,to:successor})),assign=solveLocalLayout(p,s,[target],{},preserved,extraEdges,rng);if(!assign)continue;const n=buildRepackedProgram(p,assign,s,[target],{},preserved,extraEdges);if(!n)continue;const q=pruneUnreachable(n),h=structureHealth(q);if(!h.valid)continue;return{program:q,detail:{removed:target,predecessors:preds.map(x=>x.index),successor,repacked:[...s],repackCount:s.size,removedTotal:chipCount(p)-chipCount(q)}};}return null;}
  ...
  function weaponMutation(p,rng=Math.random){const n=clone(p),ids=[...reachable(n)].filter(i=>i>0&&meta(n[i]?.type)?.kind==='action'&&meta(n[i]?.type)?.category==='weapon'),i=pick(ids,rng);if(i==null)return null;const candidates=compatible(n[i].type).filter(t=>meta(t)?.kind==='action'&&meta(t)?.category==='weapon');const type=pick(candidates,rng);if(!type)return null;n[i]={...n[i],type};return{program:n,detail:{index:i,type}};}
  ...
  function weightedOperator(rng=Math.random){let x=rng(),sum=0;for(const [name,w] of Object.entries(cfg.operatorWeights)){if(cfg.disabledOperators.includes(name)||w<=0)continue;sum+=w;if(x<=sum)return name;}return'insertConditionBranch';}
  ...
  function growSeed(target=10){let p=validSeed(),guard=0;while(chipCount(p)<Math.min(target,(typeof cpuChipLimit==='function'?cpuChipLimit():18))&&guard++<80){const r=guard%2===0?insertActionRepack(p):insertConditionBranch(p);if(!r)continue;const q=pruneUnreachable(r.program);if(structureHealth(q).valid)p=q;}return p;}
